...
const sanitizeSelection = (selection) =>
  Array.isArray(selection)
    ? selection.filter((item) => item !== undefined && item !== null)
    : [];
class CreateNC extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.navigation.state.params.data != null) {
      ...
      if (originalData.length > 0) {
        ...
        if (
          this.props.navigation.state.params.data.selectedItemsProcess === true
        ) {
        } else {
          const selectedItems =
  ...
  LongTask() {
    ...
    if (this.props.navigation.state.params.CheckpointRoute) {
      ...
      var navigationRoute = "";
      if (
        this.props.navigation.state.params.navigationfrom == "checkpointDemo"
      ) {
        navigationRoute = this.props.navigation.state.params.Formid;
      } else {
        navigationRoute =
          this.props.navigation.state.params.NCOFIDetails.Formid;
      }
      const incomingData = this.props.navigation.state.params.data;
      const sanitizedClauseSelection = sanitizeSelection(
        incomingData?.selectedItems
      );
      const sanitizedProcessSelection = sanitizeSelection(
        incomingData?.selectedItemsProcess
      );
      const sanitizedNcData = incomingData
        ? {
            ...incomingData,
            selectedItems: sanitizedClauseSelection,
            selectedItemsProcess: sanitizedProcessSelection,
          }
        : null;
      this.setState(
        {
          ...
          ncData: sanitizedNcData,
          selectedItems: sanitizedClauseSelection,
          selectedItemsProcess: sanitizedProcessSelection,
          displayData: sanitizedNcData
            ? sanitizedNcData.requiretext
            : undefined,
          NCcategoryt: sanitizedNcData
            ? sanitizedNcData.categoryDrop
            : undefined,
          NCrequestby: sanitizedNcData ? sanitizedNcData.userDrop : undefined,
          NCdept: sanitizedNcData ? sanitizedNcData.deptDrop : undefined,
          NCFailure: sanitizedNcData
            ? sanitizedNcData.failureDrop == "0"
              ? undefined
              : sanitizedNcData.failureDrop
            : undefined,
          nonconfirmityText: sanitizedNcData
            ? sanitizedNcData.NonConfirmity
            : undefined,
          NCresponsible: sanitizedNcData
            ? sanitizedNcData.requestDrop
            : undefined,
          ofitext: sanitizedNcData ? sanitizedNcData.OFI : this.state.ofitext,
          fileName: sanitizedNcData ? sanitizedNcData.filename : undefined,
          fileData: sanitizedNcData ? sanitizedNcData.filedata : undefined,
          fileSize: sanitizedNcData ? sanitizedNcData.filesize : undefined,
          ncIdentifier: sanitizedNcData
            ? sanitizedNcData.ncIdentifier
            : undefined,
          objEvidence: sanitizedNcData
            ? sanitizedNcData.objEvidence
            : undefined,
          recommAction: sanitizedNcData
            ? sanitizedNcData.recommAction
            : undefined,
          documentRef: sanitizedNcData
            ? sanitizedNcData.documentRef
            : undefined,
        },
        () => {
          this.getDropValue();
  ...
  onSelectedItemsProcessChange = (selectedItems) => {
    ...
    const multiprocess =
      this.props.navigation.state.params.NCOFIDetails.multiprocess;
    let newArry = [];
    if (multiprocess == "1") {
      ...
      this.setState({
        selectedItemsProcess: selectedItems,
        MarkProcess: false,
      });
    }
  ...
  onSelectedItemsChange = (selectedItems) => {
    const normalizedSelection = Array.isArray(selectedItems)
      ? selectedItems.filter((item) => item !== undefined && item !== null)
      : [];

    const clauses = Array.isArray(this.state.clausedata)
      ? this.state.clausedata
      : [];
    const totalObject = [];
    let requirementText = "";

    if (normalizedSelection.length && clauses.length) {
      normalizedSelection.forEach((selectedId) => {
        const clause = clauses.find((item) => item.id === selectedId);
        if (clause) {
          requirementText += clause.Requirement || "";
          totalObject.push(clause);
        }
      });
    }

    const hasRequirement = requirementText && requirementText.trim().length > 0;

    this.setState(
      {
        selectedItems: normalizedSelection,
        MarkClausedrop: false,
        displayData: hasRequirement ? requirementText : "-",
  ...
  resetForm = () => {
    this.setState(
      (prevState) => ({
        ...
        recommAction: "",
        fileArrayList: [],
        clauseSelectorKey: prevState.clauseSelectorKey + 1,
      }),
      () => {
  ...
  closeClauseSelector = () => {
    if (this.clauseListField && this.clauseListField._closeSelector) {
      this.clauseListField._closeSelector();
    }
  };

  handleClauseConfirm = () => {
    const hasClauses =
      Array.isArray(this.state.clausedata) && this.state.clausedata.length > 0;

    this.closeClauseSelector();
    this.setState((prevState) => ({
      clauseSelectorKey: prevState.clauseSelectorKey + 1,
    }));

    if (!hasClauses && this.refs.toast) {
      this.refs.toast.show(strings.NoClausesFound, DURATION.LENGTH_SHORT);
    }
  };

  openRequirementModal = (showNcText) => {
    if (!showNcText) {
      const hasClauseContent =
        Array.isArray(this.state.modalDisplay) &&
        this.state.modalDisplay.length > 0;
      if (!hasClauseContent) {
        this.refs.toast &&
          this.refs.toast.show(strings.Clauses, DURATION.LENGTH_SHORT);
        return;
      }
    } else {
      const hasText =
      ...
      if (!hasText) {
        const msg =
          this.state.RouteParam === "NC"
            ...
            : strings.OFI_Desc;
        this.refs.toast && this.refs.toast.show(msg, DURATION.LENGTH_SHORT);
        return;
      }
    }

    this.setState({ NCtxtFlag: showNcText, isVisible: true });
  };
          ...
          if (
            ...
            BundleArr = {
              ...
              NCNumber:
                ...
                "checkpointDemo"
                  ? this.state.NCNumberUpdate
                  : this.state.auditnumber,
              Category: "NC",
        ...
        if (response.size > 5000000) {
          ...
          var data = await RNFS.readFile(fileuri, "base64")
            .then((res) => {
              console.log("1:first");
              RNFetchBlob.fs
                .writeFile(newFilePath, res, "base64")
                .then((res) => {
  ...
  render() {
    ...
    const clauseItems = hasClauseData ? items : [];
    const noClauseComponent = (
      <View
        style={{
          paddingVertical: 20,
          alignItems: "center",
                    ...
                    <View style={styles.inputhigh}>
                      <View>
                        <SectionedMultiSelect
                          key={`clause-selector-${this.state.clauseSelectorKey}`}
                          ...
                          onConfirm={this.handleClauseConfirm}
                          onCancel={this.closeClauseSelector}
                          noItemsComponent={noClauseComponent}
                          styles={{
                            chipText: {
  ...
  <View style={{width:'90%',alignItems:'center',justifyContent:'center',}}>
                    ...
                    <View
                      ...
                      onPress={() => this.openRequirementModal(false)}>
                      {this.state.displayData ? (
                        <Text
                          style={{
                            padding: 0,
                      ...
                      <View style={{flexDirection: 'row'}}>
                        <TextInput
                         style={{
                          ...
                          editable={false}
                          onFocus={() => this.openRequirementModal(false)}
                        />
  ...
  <View style={{width:'20%',}}>
     <TouchableOpacity  onPress={() =>
                              this.openRequirementModal(false)
                            }>
...
bottom:10,                              backgroundColor: 'white',
                  ...
                  <TouchableOpacity style={{ right: 40,position:'absolute'}} onPress={() => this.openRequirementModal(true)}>
                    <Icon name="eye" size={20} color="black" />
