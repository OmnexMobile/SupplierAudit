...
class AuditDashboardFooter extends Component {
  ...
  chooseAction(index) {
    const isSupplierEnabled = this.props.data.audits.suppliermanagementstatus === 'true';
  
    // Cancel button: Do nothing
    if (index === (isSupplierEnabled ? 7 : 6)) {
      return;
    }
    ...
    } else if (index === 5 && isSupplierEnabled) {
      this.props.navigation.navigate('SupplyManage');
    } else if (
      (index === 5 && !isSupplierEnabled) ||
      (index === 6 && isSupplierEnabled)
    ) {
